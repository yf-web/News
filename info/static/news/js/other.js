...
function getNewsList(page) {
    var query = decodeQuery();  // 解析url中的值
    var params = {
        "p": page,
        "user_id": query["other_id"]
    };
    $.get("/user/other_news_list", params, function (resp) {
        if (resp.errno == "0") {
            ...
            $(".article_list").html("");
            // 拼接数据
            for (var i = 0; i<resp.data.other_news_li.length; i++) {
                var news = resp.data.other_news_li[i];
                var html = '<li><a href="/news/'+ news.id +'" target="_blank">' + news.title + '</a><span>' + news.create_time + '</span></li>';
                // 添加数据
                $(".article_list").append(html)
            }
            // 设置页数和总页数
            $("#pagination").pagination("setPage", resp.data.current_page, resp.data.total_pages);
        }else {
            alert(resp.errmsg)
        }
    })
